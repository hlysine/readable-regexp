...
describe('charRange', () => {
  it('accepts two strings', () => {
    expect(charRange`a``z`.toString()).toBe('[a-z]');
    expect(charRange('a')('z').toString()).toBe('[a-z]');
    expect(charRange`a`('z').toString()).toBe('[a-z]');
    expect(charRange('a')`z`.toString()).toBe('[a-z]');
    expect(charRange('a', 'z').toString()).toBe('[a-z]');
  ...
  it('validates range order', () => {
    expect(charRange`a``z`.toString()).toBe('[a-z]');
    expect(() => charRange`z``a`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\a``\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\n``\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\n``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\cE``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\123``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\567``\\0`.toString()).toThrow('octal escape');
    expect(() => charRange`\\999``\\0`.toString()).toThrow('single character');
    expect(() => charRange`\\\\``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\s``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\u{093f4e}``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\u{3ef4e2}``\\0`.toString()).toThrow('unicode escape');
    expect(() => charRange`\xef``\x32`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\xef``\\x32`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\xfff``\\x32`.toString()).toThrow('hex escape');
    expect(() => charRange`\uef12``\u2314`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\uef12``\\u2314`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`\\u093ef4``\\u2314`.toString()).toThrow('unicode escape');
    expect(() => charRange`÷``×`.toString()).toThrow('Invalid charRange');
    expect(() => charRange`👍``👋`.toString()).toThrow('Invalid charRange');
  });
  it('works with nested char classes', () => {
    expect(charIn`a-z`(charRange`A``Z`).toString()).toBe('[a-zA-Z]');
    expect(charIn`a-z${charRange`A``Z`}`.toString()).toBe('[a-zA-Z]');
    ...
    expect(notCharIn`a-z`(charRange`A``Z`).toString()).toBe('[^a-zA-Z]');
  });
  it('escapes properly', () => {
    expect(charRange`\\``a`.toString()).toBe('[\\\\-a]');
    expect(charRange`$``\\`.toString()).toBe('[$-\\\\]');
    expect(charRange`^``_`.toString()).toBe('[\\^-_]');
    expect(charRange`[``]`.toString()).toBe('[[-\\]]');
  });
  it('is chainable and quantifiable', () => {
    expect(charRange`a``c`.char.toString()).toBe('[a-c].');
    expect(oneOrMore.charRange`a``b`.toString()).toBe('[a-b]+');
    expect(oneOrMore.charRange('a', 'b').toString()).toBe('[a-b]+');
    expect(oneOrMore.charRange`${tab}``${verticalWhitespace}`.toString()).toBe('[\\t-\\v]+');
  });
  it('can be negated', () => {
    expect(not.charRange`a``z`.toString()).toBe('[^a-z]');
    expect(not(charRange`a``z`).toString()).toBe('[^a-z]');
    expect(notCharRange`a``z`.toString()).toBe('[^a-z]');
  });
  it('throws for invalid argument', () => {
    // @ts-expect-error - testing invalid arguments
    expect(() => charRange().toString()).toThrow();
    // @ts-expect-error - testing invalid arguments
    expect(() => charRange``.toString()).toThrow();
    // @ts-expect-error - testing invalid arguments
...
describe('notCharRange', () => {
  ...
  it('validates range order', () => {
    ...
    expect(() => notCharRange`\n``\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\n``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\cE``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\123``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\567``\\0`.toString()).toThrow('octal escape');
    expect(() => notCharRange`\\999``\\0`.toString()).toThrow('single character');
    expect(() => notCharRange`\\\\``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\s``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\u{093f4e}``\\0`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\u{3ef4e2}``\\0`.toString()).toThrow('unicode escape');
    expect(() => notCharRange`\xef``\x32`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\xef``\\x32`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\xfff``\\x32`.toString()).toThrow('hex escape');
    expect(() => notCharRange`\uef12``\u2314`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\uef12``\\u2314`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`\\u093ef4``\\u2314`.toString()).toThrow('unicode escape');
    expect(() => notCharRange`÷``×`.toString()).toThrow('Invalid charRange');
    expect(() => notCharRange`👍``👋`.toString()).toThrow('Invalid charRange');
  });
